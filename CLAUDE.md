...
You are expected to work autonomously. Read the repository, establish facts
for yourself, and carry out the build plan in section 11 without asking
permission at each step.

- **Resolve uncertainty by reading, not by asking.** Almost every open
  question about this codebase is answerable from the code. Go and find out.
- **Change only what the current task covers.** No extra arguments,
  conditions, fallbacks, or features nobody requested. No refactoring on the
  side.
...
- **Be concise. Work step by step.** State assumptions separately from
  conclusions.

### The only reasons to stop and ask the owner

1. A task appears to require breaking a hard rule in section 2.
2. A task appears to require editing a file in section 5.
3. `DECISIONS.md` and `ARCHITECTURE.md` contradict each other on a fact the
   task depends on.
4. A design cannot meet the 20-second target in section 8.
5. Something in the repository flatly contradicts a recorded decision.

Anything else: decide, record your reasoning in the commit or the findings
file, and carry on.
...
Work through these in order. Do not start a stage before the one it depends
on. Report at the end of each stage; do not wait for permission to begin the
next.

### Stage 1: Reconnaissance

Read the repository and establish these facts for yourself. They were left out
of this file deliberately, because the code is the only honest source.
...
4. **CSS.** Is there an existing stylesheet, panel style, or modal style the
   assistant's panel should follow? Anything resembling a design system, or is
   styling ad hoc?
5. **Tests.** Any tests in the repository at all? Any test runner?
6. **The assistant scaffolding.** What do `assistant/state_observer.js`,
   ...
   contain, and how far do they match what `DECISIONS.md` records as decided?

Write findings to `assistant/FINDINGS.md`. For each: what you found, the file
and location, and plainly where you could not determine it. Change no code in
this stage.

### Stage 2: The resolvers

Pure lookup functions over the collections already cached at load. Given a
gene name, return its entry or nothing. Given a population label, return its
record or nothing. Exact match only. No fuzzy matching, no normalising.
Multiple matches return all of them so the caller can ask the user.

These are pure, DELPHI-free, and testable without a model. Test them.

### Stage 3: The action layer

One thin file of parameterised, DOM-free action functions. Each one:

- validates its parameters before doing anything;
- writes to `site_options` through `getOptions`;
- dispatches `update` or `refresh` on `[data-module="browser"]`;
- verifies afterwards that the state actually changed;
- returns a typed result the caller can reason about, never a bare throw.

Separate functions for adding and replacing populations. No mode flags, no
catchall arguments.

Coordinate validation lives here, against `CHR_LENGTHS` keyed by chromosome.

### Stage 4: The panel

The new element in `index.html`, the new top-level entry module, and the panel
shell. Follow whatever CSS conventions stage 1 found. Output via `textContent`
only. Keyboard operable. Focus moves into the panel when it opens.

No model yet. A text box that routes typed commands straight to the action
layer is the correct state at the end of this stage, and it proves the whole
deterministic path works before a model is anywhere near it.

### Stage 5: The model loop

WebLLM, loaded once at startup, not per request. Output constrained so the
model can only emit a valid command. One decision per call. Short outputs.

Code routes between the model, the resolvers, and the action layer. Code
decides what to say when something fails, per section 6.

### Stage 6: Measure

Per-capability success rate and per-capability wall-clock time, on this
machine, with DELPHI running in the same tab. Then describe each capability as
the number it actually is. A capability that succeeds six times in ten is a
six-in-ten capability and is described that way.
