...
export const COMMAND_SCHEMA = {
	anyOf: [
		actionSchema('gene', { gene_name: { type: 'string' } }),
		actionSchema('region', { chr: { type: 'string', enum: CHROMOSOMES }, start: { type: 'integer' }, end: { type: 'integer' } }),
		actionSchema('statistic', { measure: { type: 'string', enum: MEASURES } }),
		actionSchema('add_population', { population_label: { type: 'string' } }),
		actionSchema('replace_population', { population_label: { type: 'string' } }),
		actionSchema('sort', { sort_field: { type: 'string' }, sort_direction: { type: 'string', enum: SORT_DIRECTIONS } }),
		actionSchema('question', { field: { type: 'string', enum: STATE_FIELDS } }),
