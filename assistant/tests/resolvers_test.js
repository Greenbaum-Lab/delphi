...
const CASES = [
	...
	['a one-letter typo is offered back', () => resolvePopulation(POPULATION_RECORDS, 'Finish').candidates.includes('Finnish')],
	['a one-letter typo still does not resolve', () => resolvePopulation(POPULATION_RECORDS, 'Finish').status === 'ambiguous'],
	['a two-letter typo is not offered', () => resolvePopulation(POPULATION_RECORDS, 'Finsh').status === 'not_found'],
	['a short query is not typo-matched', () => resolveGene(GENE_MAP, 'MCM').candidates.includes('MCM6')],
